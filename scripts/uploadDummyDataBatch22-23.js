/**
 * Script to upload dummy data for batches 22-23
 * Starting date: December 2, 2025
 * Run with: node scripts/uploadDummyDataBatch22-23.js
 * 
 * Creates 2 batches (0022-0023) with 30-60 eggs each
 * Processing time: 11 seconds per egg
 */
...
// Helper to pad batch number (e.g., 22 -> "0022")
function padBatchNumber(num) {
  return String(num).padStart(4, '0')
}

async function uploadDummyData() {
  console.log('🚀 Starting dummy data generation for batches 22-23...')
  console.log(`📦 Account ID: ${ACCOUNT_ID}`)
  console.log(`🎯 Creating ${NUM_BATCHES} batches (0022-0023)`)
  console.log(`📅 Start date: December 2, 2025`)
  ...
  try {
    ...
    // Start date: December 2, 2025, 8:00 AM
    const startDate = new Date('2025-12-02T08:00:00')
    
    // Hours between batches (spread them across the day)
    const hoursBetweenBatches = 6 // 6 hours between batches
    
    let currentTime = new Date(startDate)

    // Generate batches 22 and 23
    for (let i = 0; i < NUM_BATCHES; i++) {
      const batchNum = START_BATCH_NUM + i
      ...
      console.log(`📦 Generating Batch ${batchNum} (${i + 1}/${NUM_BATCHES}): ${batchId} with ${numEggs} eggs...`)
      ...
      const batchEggs = Array.from({ length: numEggs }, (_, eggIndex) => 
        generateRandomEgg(batchNumber, batchStartTime, eggIndex, numEggs)
      )
      ...
      const formattedDate = batchStartTime.toLocaleDateString('en-US', { 
        month: 'short', 
        day: 'numeric',
        year: 'numeric',
        hour: '2-digit',
      ...
      // Move to next batch start time (6 hours later)
      currentTime = new Date(currentTime.getTime() + (hoursBetweenBatches * 60 * 60 * 1000))
    }
    ...
    allBatches.forEach(b => {
      ...
      console.log(`   ${b.id}: ${s.totalEggs} eggs in ${mins}min (Good: ${s.goodEggs}, Dirty: ${s.dirtyEggs}, Cracked: ${s.crackEggs}) - ${dateStr}`)
    })
